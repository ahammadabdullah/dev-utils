...
export const useGitignoreGenerator = () => {
  ...
  const templatesByCategory = useMemo(() => {
    const categories = ['languages', 'frameworks', 'tools', 'os', 'editors'] as const;
  ...
  const generateGitignore = () => {
    console.log('Generating gitignore with selected templates:', state.selectedTemplates);
    ...
    console.log('Found template objects:', selectedTemplateObjects.map(t => t.name));
  ...
  const downloadGitignore = () => {
    ...
    try {
      console.log('Downloading gitignore with content length:', state.generatedContent.length);
      ...
      try {
        const dataStr = 'data:text/plain;charset=utf-8,' + encodeURIComponent(state.generatedContent);
        const a = document.createElement('a');
        a.href = dataStr;
        a.download = '.gitignore';
        a.click();
